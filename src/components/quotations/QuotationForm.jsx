...
import { createQuotation, updateQuotation } from "../../actions/quotationsController"
import { useSearchParams } from "next/navigation"
import ClientSelector from "./ClientSelector"
import LineItemEditor from "./LineItemEditor"
import Link from "next/link"

export default function QuotationForm({ clients, quotation = null }) {
    ...
    const action = isEditing 
        ? updateQuotation.bind(null, quotation._id)
        : createQuotation
    
    const [formState, formAction] = useActionState(action, {})
    
    const [selectedClientId, setSelectedClientId] = useState(
        quotation?.clientId || preSelectedClientId || ""
    )
    const [items, setItems] = useState(
        quotation?.items || [{ description: "", quantity: 1, unitPrice: 0 }]
    )
    const [taxRate, setTaxRate] = useState(quotation?.taxRate || 15)
    ...
    const [quotationDate, setQuotationDate] = useState(
        quotation?.date ? new Date(quotation.date).toISOString().split('T')[0] : today
    )
    const [validUntil, setValidUntil] = useState(() => {
        if (quotation?.validUntil) {
            return new Date(quotation.validUntil).toISOString().split('T')[0]
        }
        const date = new Date()
    ...
    return (
        <form action={handleSubmit} className="space-y-6">
                    ...
                    <div className="form-control">
                        ...
                        {formState.errors?.validUntil && (
                            <div role="alert" className="alert alert-warning mt-2 flex items-center gap-2 rounded-xl">
                                ...
                                <span>{formState.errors.validUntil}</span>
                            </div>
                        )}
                    </div>
                </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4">
                ...
                <Link
                    href={isEditing ? `/quotations/${quotation._id}` : "/quotations"}
